...
import { GeoPricingTabs } from '@/components/geo/GeoPricingTabs';
import { GeoProofSection } from '@/components/geo/GeoProofSection';
import { GeoFaqSection } from '@/components/geo/GeoFaqSection';
import { InfoShowcaseSection } from '@/components/InfoShowcaseSection';

const steps = [
  { number: "ШАГ 01", title: "Карта запросов" },
  { number: "ШАГ 02", title: "Два прогона и разметка" },
  { number: "ШАГ 03", title: "Источники и сайт" },
  { number: "ШАГ 04", title: "Отчет и план работ" },
  { number: "ШАГ 05", title: "Съем через месяц" }
];
...
const quoteLines = [
  // Строки короткие: на десктопе они идут в одну строку без переноса (md:whitespace-nowrap),
  // кириллица в Oswald шире латиницы, и длинные строки уезжали за край экрана
  "МОЯ РАБОТА ПРОСТАЯ — замерить, что нейросети",
  "отвечают покупателю, найти <span class='text-[#c7b684]'>управляемые причины</span>",
  "и поработать с ними. Позиций в ответах нейросетей",
  "не гарантирует никто. Поэтому я показываю <span class='text-[#c7b684]'>динамику</span>",
  "повторным съёмом, а не обещаю результат"
];
              ...
              >
                ...
                <div className="w-full border-t border-b border-[#e0ded8]/20 hidden lg:block">
                  <div className="grid grid-cols-[15vw_22.5vw_22.5vw] gap-0">
                    {[
                      { label: "КРИТЕРИЙ", active: false },
                      { label: "АНТОН КОЛЕСНИКОВ", active: true },
                      { label: "АВТООТЧЁТ ЗА 3–5 ТЫС.", active: false }
                    ].map((header, i) => (
                      <div
                        key={header.label}
                  ...
                  {comparisonData.map((item, idx) => (
                    <div key={idx} className="flex flex-col space-y-3 border-b border-[#e0ded8]/10 pb-6">
                      ...
                      <div className="bg-[#e0ded8]/5 p-4 rounded-lg">
                        <span className="text-[2.5vw] font-mono text-[#e0ded8]/40 uppercase block mb-1">Антон Колесников</span>
                        <p className="text-[3.5vw] font-mono font-bold text-[#e0ded8]">{item.me}</p>
                      </div>
                      <div className="p-4 border border-[#e0ded8]/5 rounded-lg">
                        <span className="text-[2.5vw] font-mono text-[#e0ded8]/40 uppercase block mb-1">Автоотчёт за 3–5 тыс.</span>
        ...
        <div className="relative w-full aspect-[21/9] min-h-[500px] bg-black overflow-hidden flex items-center justify-center">
          <div className="absolute inset-0 p-2 md:p-[1vw]">
             <div className="relative w-full h-full overflow-hidden border border-white/10 group">
                ...
                <div className="absolute top-4 left-4 font-mono text-[10px] text-white/30 uppercase tracking-widest">[ АНТОН КОЛЕСНИКОВ ]</div>
                <div className="absolute top-4 right-4 font-mono text-[10px] text-white/30 uppercase tracking-widest">[ 2026 ]</div>
             </div>
          </div>
        </div>
      </section>
